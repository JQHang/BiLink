#!/usr/bin/env python3
"""
Prepare Task Data for Author-New-Coauthor Link Discovery

This script generates the necessary tables for the author-coauthor prediction task.
These tables are used throughout the entire pipeline: training, validation, testing,
inference, and bipath exploration.

Tables Generated:
1. known_target_edges - Author-author coauthor pairs with train/valid/test/infer splits
2. exist_target_edges - Authors' existing coauthorships (for filtering already-known relationships)
3. candidate_authors - Active authors (published in past 5 years)

Usage:
    python prepare_task_data.py \\
        --bilink-config main/config/bilink/author_new_coauthor/21_to_24_example.yaml \\
...
def compute_author_first_coauthor(spark_ctx, works_author_path,
                                  candidate_authors_path):
    """
    Compute first coauthorship date for each author-coauthor pair and load candidate authors.
    ...
    Returns:
        Tuple of (author_first_coauthor_df, author_work_df, candidate_authors_df)
        all cached for downstream use
    """
    logger.info("Computing shared author first coauthor data...")
    ...
    # Prepare coauthor view with renamed columns
    coauthor_work_df = author_work_df.select(
        col("author_id").alias("coauthor_id"),
        col("work_id"),
        col("date")
    )

    # Self-join to find coauthors (authors who worked on same papers)
    coauthor_df = author_work_df.join(
        coauthor_work_df,
        on=["work_id", "date"],
        how="inner"
    ).select(
    ...
    # Filter out self-collaborations (author_id == coauthor_id)
    coauthor_df = coauthor_df.filter(col("author_id") != col("coauthor_id"))

    # Remove duplicates
    coauthor_df = coauthor_df.distinct()

    # Keep only first coauthorship per author-coauthor pair
    author_first_coauthor_df = ordered_sample(
        coauthor_df,
        group_columns=["author_id", "coauthor_id"],
    ...
    spark_ctx.persist_manager.persist(
        author_first_coauthor_df,
        release_point='target_edges_done',
        name='author_first_coauthor'
    )
    ...
    logger.info("Shared author first coauthor data and candidate authors cached")

    return author_first_coauthor_df, author_work_df, candidate_authors_df


# ==================== Table Generation Functions ====================

def generate_known_target_edges(spark_ctx, author_first_coauthor_df, author_work_df,
                               candidate_authors_df, known_target_edges_path,
                               sample_type_to_date_range):
    """
    Generate known_target_edges table.

    This table contains author-author coauthor pairs with train/valid/test/infer splits.
    Each row represents the first time an author collaborated with a specific coauthor.

    Processing steps:
    1. Use pre-computed author_first_coauthor_df (passed as parameter)
    2. Add sample_type labels based on date ranges
    3. Filter to only active author pairs (both must be candidates) using pre-computed candidate_authors DataFrame
    4. Normalize dates to Jan 1st of each year

    Args:
        spark_ctx: SparkContext from SparkRunner
        author_first_coauthor_df: Pre-computed first coauthorship DataFrame (cached)
    ...
    label_df = author_first_coauthor_df.withColumn("sample_type", sample_type_expr)
    ...
    # Filter to only active author-coauthor pairs (both must be in candidate_authors)
    # First filter by author_id (u_node)
    ...
    # Then filter by coauthor_id (v_node)
    candidate_v_node_df = candidate_authors_df.select(
        col("author_id").alias("coauthor_id"),
        col("date")
    )
    label_df = label_df.join(
...
def generate_exist_target_edges(spark_ctx, author_first_coauthor_df, author_work_df,
                               candidate_authors_df, exist_target_edges_path, target_dates):
    """
    Generate exist_target_edges table.

    This table contains coauthors that authors have already collaborated with before each query date.
    It's used to filter out known relationships during bipath exploration.

    Processing steps:
    1. Use pre-computed author_first_coauthor_df (passed as parameter)
    2. For each query date, include collaborations established before that date
    3. Filter to only active author pairs (both must be candidates) using pre-computed candidate_authors DataFrame
    ...
    author_first_coauthor_df = author_first_coauthor_df.withColumnRenamed('date', 'first_date')
    ...
    exist_edge_df = author_first_coauthor_df.join(
        broadcast(time_df),
        col('first_date') < col('date'),
        how='inner'
    ).select(
        col('author_id').alias('u_node_id'),
        col('coauthor_id').alias('v_node_id'),
        col('date')
    ).distinct()

    # No self-work union needed - self-relationships already filtered in compute_author_first_coauthor

    # Filter to only active author pairs (both must be in candidate_authors)
    # First filter by u_node_id (author_id)
    candidate_u_node_df = candidate_authors_df.select(
        col("author_id").alias("u_node_id"),
        col("date")
    )
    ...
    # Then filter by v_node_id (coauthor_id)
    candidate_v_node_df = candidate_authors_df.select(
        col("author_id").alias("v_node_id"),
        col("date")
    )
    exist_edge_df = exist_edge_df.join(
        candidate_v_node_df,
...
def prepare_all_task_data(spark_ctx, context_table_dir, task_table_dir,
    ...
    3. If needed, compute shared author_first_coauthor data once
    ...
    if not known_complete or not exist_complete:
        # Compute shared data (author first coauthor + candidate authors)
        logger.info("\n[2/3] Computing shared data...")
        author_first_coauthor_df, author_work_df, candidate_authors_df = compute_author_first_coauthor(
            spark_ctx,
            works_author_path,
            candidate_authors_path
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Prepare task data for author-new-coauthor link discovery',
